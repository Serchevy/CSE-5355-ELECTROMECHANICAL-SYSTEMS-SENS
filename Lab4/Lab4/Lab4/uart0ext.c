...
char* itoa(uint32_t num)
{
    ...
    {
        // isolates each digit into num array, in reverse order
        while(num)
        {
            numChar[i] = num%10;
            num/=10;
        ...
        // corrects order and converts to ASCII
        while(i>-1)
        {
            //putcUart0(numChar[i]+48);
            strNum[strIndex] = (char)(numChar[i]+48);
            strIndex++;
            i--;
        }
        strNum[strIndex] = '\0';
